...
import { buildImplementationPrompt } from '@/lib/ai/prompts/implementation';
import type { GenerateRequest } from '@/types/api';
import type { Feature, ClarifiedIdea, MVPScope } from '@/types/workspace';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = JSON.parse(extractJSON(textBlock.text)) as Feature[];

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error('Implementation prompts API error:', error);
    return NextResponse.json(
      {
        success: false,
